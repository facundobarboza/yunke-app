-- =============================================================================
-- MIGRATION: Migrar is_admin a sistema RBAC
-- =============================================================================
-- Asigna el rol 'Dirigente' a todos los usuarios que tienen is_admin = true.
-- Idempotente: safe to run multiple times (ON CONFLICT DO NOTHING).
-- Si la columna is_admin no existe, la migración se salta silenciosamente.
-- =============================================================================

DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'profiles' AND column_name = 'is_admin'
  ) THEN
    INSERT INTO user_roles (user_id, role_id, assigned_at)
    SELECT p.id, r.id, NOW()
    FROM profiles p
    JOIN roles r ON r.name = 'Dirigente'
    WHERE p.is_admin = true
    ON CONFLICT (user_id, role_id) DO NOTHING;

    RAISE NOTICE 'Migración completada: usuarios admin asignados al rol Dirigente';
  ELSE
    RAISE NOTICE 'Columna is_admin no existe — saltando migración';
  END IF;
END $$;
